return {
  ...
  opts = {
    server = {
      -- Absolute mason path: PATH rust-analyzer is a rustup shim that recurses.
      cmd = function()
        return { vim.fn.stdpath("data") .. "/mason/bin/rust-analyzer" }
      end,
      ...
      default_settings = {
        ["rust-analyzer"] = {
          cargo = {
            ...
            -- Isolate RA artifacts from `cargo run` / `just watch`.
            -- Sharing target/debug dirties gpui/wayland build scripts
            -- (CC/PKG_CONFIG differ: nvim vs nix develop) and takes ~7–10s.
            targetDir = true,
          },
          checkOnSave = true,
          check = {
