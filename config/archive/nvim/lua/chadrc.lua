      ...
      buffers = function()
        ...
        for _, nr in ipairs(vim.t.bufs) do
          if api.nvim_buf_is_valid(nr) then
            ...
            local is_curbuf = nr == current
            local modified = vim.bo[nr].modified
            local hl = "BufO" .. (is_curbuf and "n" or "ff")
            local mod_hl = "BufO" .. (is_curbuf and "n" or "ff") .. "Modified"

            local icon = "󰈚 "
            local devicon = require("nvim-web-devicons").get_icon(name)
            if devicon then
              icon = " " .. devicon .. " "
            end

            local name_txt = txt(icon .. name .. " ", hl)
            if modified then
              name_txt = name_txt .. txt(" ", mod_hl)
            end

            local buf_btn = btn(name_txt, nil, "GoToBuf", nr)
            table.insert(buffers, buf_btn)
          end
        end

        return table.concat(buffers) .. "%=%#TbFill#"
      end,
    },
  },
}
